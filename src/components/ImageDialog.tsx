...
// ImageDialog Props TypeScript ?∏ÌÑ∞?òÏù¥???ïÏùò
...
// ?¥ÌåÅ Í∑∏Î£π Ïª¥Ìè¨?åÌä∏Î•??ïÏùò?òÏó¨ ÏΩîÎìúÎ•?Í∞ÑÍ≤∞??(onClick prop Ï∂îÍ?)
const ActionTooltip = ({
  icon,
  label,
  onClick,
...
export function ImageDialog({ props }: { props: ImageDialogProps }) {
  ...
  const { mutate: postComment } = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("?ìÍ????±Î°ù?òÏóà?µÎãà??");
    },
    onError: () => {
      toast.error("?ìÍ? ?±Î°ù???§Ìå®?àÏäµ?àÎã§.");
    }
  });

  const handleLikeToggle = () => {
    if (!currentUser) {
      if (confirm("Î°úÍ∑∏?∏Ïù¥ ?ÑÏöî??Í∏∞Îä•?ÖÎãà?? Î°úÍ∑∏?∏Ìïò?úÍ≤†?µÎãàÍπ?")) {
      ...
      >
        <div className="absolute top-4 right-4 z-10 flex flex-col gap-3">
          <ActionTooltip
            ...
            label={displayLikes > 0 ? addCommas(displayLikes) : "Ï¢ãÏïÑ??}
          ...
          />
          <ActionTooltip icon={<User size={20} />} label="?ÑÎ°ú?? />
          <ActionTooltip icon={<Send size={20} />} label="?úÏïà?òÍ∏∞" />
          <ActionTooltip icon={<Download size={20} />} label="?§Ïö¥Î°úÎìú" />
        </div>
        ...
        <div className="p-6">
          <DialogHeader>
            ...
            <DialogDescription>
              {props.description || "?±Î°ù???§Î™Ö???ÜÏäµ?àÎã§."}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-start gap-2">
                ...
                <p className="text-neutral-500 line-clamp-3">
                  {props.alt_description || "?±Î°ù???§Î™Ö???ÜÏäµ?àÎã§."}
                </p>
              </div>
